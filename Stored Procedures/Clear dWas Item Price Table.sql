...
ALTER PROCEDURE [dbo].[Clear dWas Item Price]
AS
BEGIN
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SET @Row_Count = (
                SELECT COUNT(*)
                FROM [Warehouse].[dbo].[dWas Item Price]
                );
        ...
        */
        ...
        BEGIN
            SELECT @Timestamp = GETDATE()
                ,@Level = 'ERROR'
                ,@File_Name = 'Clear dWas Item Price Table.sql'
                ,@Table = 'dWas Item Price'
                ,@Action = 'delete all data from dWas Item Price table.'
                ,@Row_Count = @Row_Count
                ,@Start_Date = NULL
                ,@End_Date = NULL
                ,@Message = CAST((
