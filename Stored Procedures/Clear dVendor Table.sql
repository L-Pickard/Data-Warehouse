...
ALTER PROCEDURE [dbo].[Clear dVendor Table]
AS
BEGIN
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SET @Row_Count = (
                SELECT COUNT(*)
                FROM [Warehouse].[dbo].[dVendor]
                );

        ALTER TABLE [Warehouse].[dbo].[fPurchases] NOCHECK CONSTRAINT ALL;

        ALTER TABLE [Warehouse].[dbo].[dCountry] NOCHECK CONSTRAINT ALL;
        ...
        ALTER TABLE [Warehouse].[dbo].[dCountry] CHECK CONSTRAINT ALL;
        ...
        */
        ...
        BEGIN
            SELECT @Timestamp = GETDATE()
                ,@Level = 'ERROR'
                ,@File_Name = 'Clear dVendor Table.sql'
                ,@Table = 'dCountry'
                ,@Action = 'delete all data from dVendor table.'
                ,@Row_Count = @Row_Count
                ,@Start_Date = NULL
                ,@End_Date = NULL
                ,@Message = CAST((
