...
ALTER PROCEDURE [dbo].[Clear fAdvert Spend Table]
AS
BEGIN
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SET @Row_Count = (
                SELECT COUNT(*)
                FROM [Warehouse].[dbo].[fAdvert Spend]
                );
        ...
        */
        ...
        BEGIN
            SELECT @Timestamp = GETDATE()
                ,@Level = 'ERROR'
                ,@File_Name = 'Clear fAdvert Spend table.sql'
                ,@Table = 'fAdvert Spend'
                ,@Action = 'delete all data from fAdvert Spend table.'
                ,@Row_Count = @Row_Count
                ,@Start_Date = NULL
                ,@End_Date = NULL
                ,@Message = CAST((
