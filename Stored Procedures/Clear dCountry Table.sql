...
ALTER PROCEDURE [dbo].[Clear dCountry Table]
AS
BEGIN
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SET @Row_Count = (
                SELECT COUNT(*)
                FROM [Warehouse].[dbo].[dCountry]
                );

        ALTER TABLE [Warehouse].[dbo].[fSales] NOCHECK CONSTRAINT ALL;

        ALTER TABLE [Warehouse].[dbo].[dCustomer] NOCHECK CONSTRAINT ALL;

        ALTER TABLE [Warehouse].[dbo].[dVendor] NOCHECK CONSTRAINT ALL;
        ...
        ALTER TABLE [Warehouse].[dbo].[dCustomer] CHECK CONSTRAINT ALL;

        ALTER TABLE [Warehouse].[dbo].[dVendor] CHECK CONSTRAINT ALL;
        ...
        */
        ...
        BEGIN
            SELECT @Timestamp = GETDATE()
                ,@Level = 'ERROR'
                ,@File_Name = 'Clear dCountry Table.sql'
                ,@Table = 'dCountry'
                ,@Action = 'delete all data from dCountry table.'
                ,@Row_Count = @Row_Count
                ,@Start_Date = NULL
                ,@End_Date = NULL
                ,@Message = CAST((
