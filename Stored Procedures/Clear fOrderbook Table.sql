...
ALTER PROCEDURE [dbo].[Clear fOrderbook Table]
AS
BEGIN
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
        ...
        */
        ...
        BEGIN
            SELECT @Timestamp = GETDATE()
                ,@Level = 'ERROR'
                ,@File_Name = 'Clear fOrderbook Table.sql'
                ,@Table = 'fOrderbook'
                ,@Action = 'delete all data from fOrderbook table.'
                ,@Row_Count = @Row_Count
                ,@Start_Date = NULL
                ,@End_Date = NULL
                ,@Message = CAST((
