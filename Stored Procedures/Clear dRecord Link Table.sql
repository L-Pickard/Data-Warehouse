...
ALTER PROCEDURE [dbo].[Clear dRecord Link Table]
AS
BEGIN
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SET @Row_Count = (
                SELECT COUNT(*)
                FROM [Warehouse].[dbo].[dRecord Link]
                );
        ...
        */
        ...
        BEGIN
            SELECT @Timestamp = GETDATE()
                ,@Level = 'ERROR'
                ,@File_Name = 'Clear dRecord Link Table.sql'
                ,@Table = 'dCountry'
                ,@Action = 'delete all data from dRecord Link table.'
                ,@Row_Count = @Row_Count
                ,@Start_Date = NULL
                ,@End_Date = NULL
                ,@Message = CAST((
