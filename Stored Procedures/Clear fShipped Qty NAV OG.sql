...
ALTER PROCEDURE [dbo].[Clear fShipped Qty Table NAV OG]
AS
BEGIN
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SET @Row_Count = (
                SELECT COUNT(*)
                FROM [Warehouse].[dbo].[fShipped Qty NAV OG]
                );
        ...
        */
        ...
        BEGIN
            SELECT @Timestamp = GETDATE()
                ,@Level = 'ERROR'
                ,@File_Name = 'Clear fShipped Qty NAV OG Table.sql'
                ,@Table = 'fShipped Qty'
                ,@Action = 'delete all data from fShipped Qty NAV OG table.'
                ,@Row_Count = @Row_Count
                ,@Start_Date = NULL
                ,@End_Date = NULL
                ,@Message = CAST((
