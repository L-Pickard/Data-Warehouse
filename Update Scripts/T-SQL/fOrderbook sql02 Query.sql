/*===============================================================================================================================================
Project: Data Warehouse - fOrderbook UK/EU
Language: T-SQL
Author: Leo Pickard
Version: 1.0
Date: 31/07/2024
=================================================================================================================================================
This query will run against UK/EU server each day to update the forderbook table in warehouse db.
================================================================================================================================================*/
WITH Ltd_Reservations
AS (
	SELECT CASE 
	...
	FROM [Entry Reservation] AS re
	
	LEFT JOIN [Entry Reservation] AS rs
...
AS (
	...
	FROM [EU Entry Reservation] AS re
	
	LEFT JOIN [EU Entry Reservation] AS rs
		ON re.[Entry No_] = rs.[Entry No_]
			AND re.[Item No_] = rs.[Item No_]
			AND re.[Location Code] = rs.[Location Code]
			AND rs.[Source Type] <> 37
...
AS (
	...
	FROM [Line Sales] AS sl
	
	LEFT JOIN [Header Sales] AS sh
		ON sl.[Document No_] = sh.[No_]
	
	LEFT JOIN Ltd_Reservations AS ir
	...
	LEFT JOIN Ltd_Reservations AS pr
	...
	FROM [EU Line Sales] AS sl
	
	LEFT JOIN [EU Header Sales] AS sh
		ON sl.[Document No_] = sh.No_
	
	LEFT JOIN BV_Reservations AS ir
		ON sl.[No_] = ir.[Item No]
			AND sl.[Location Code] = ir.[Location Code]
			AND sl.[Document No_] = ir.[Source ID]
			AND ir.[Reservation Type] = 'Item Ledger'
	
	LEFT JOIN BV_Reservations AS pr
		ON sl.[No_] = pr.[Item No]
			AND sl.[Location Code] = pr.[Location Code]
			AND sl.[Document No_] = pr.[Source ID]
			AND pr.[Reservation Type] = 'Purchase Order'
