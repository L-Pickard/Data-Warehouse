/*================================================================================================================
Project: Data Warehouse - Org LLC GL Entry Table
Author: Leo Pickard
Date: April 2024
Version: 1.00

This SQL select statement gets the data needed for the GL entry table in the finance database on sql18. This
...
-- SET @DTStart = DATEADD(DAY, 1, '2024-01-01')
...
SELECT CAST(CAST(YEAR(gl.[Posting Date]) AS VARCHAR(4)) + RIGHT('0' + CAST(MONTH(gl.
    ...
    ,CAST(gl.[Amount] AS DECIMAL(20, 8)) AS 'USD Amount'
    ,CAST(gl.[VAT Amount] AS DECIMAL(20, 8)) AS 'USD VAT Amount'
    ,CAST(gl.[Debit Amount] AS DECIMAL(20, 8)) AS 'USD Debit Amount'
    ,CAST(gl.[Credit Amount] AS DECIMAL(20, 8)) AS 'USD Credit Amount'
    ,CAST(gl.[Posting Date] AS DATE) AS 'USD XR Date'
    ,CAST(1.00 AS DECIMAL(12, 8)) AS 'USD XR'
FROM [US GL Entry] AS gl
WHERE [Posting Date] BETWEEN @DTStart
        AND @DTEnd