...
AS (
    SELECT it.[Item No]
        ,TRIM(rl.[URL]) AS [Img URL]
        ,TRIM(rl.[Description]) AS [File Name]
        ,CASE 
...
SELECT ri.[Item No]
    ...
    ,it.[Tariff No]
    ,it.[COO]
    ,ri.[File Name] AS [Old File Name]
    ,ri.[Image Path]
FROM [Required Images] AS ri
LEFT JOIN [dItem] AS it
    ON ri.[Item No] = it.[Item No]
WHERE ri.[Item No] NOT IN (
        SELECT [Item No]
        FROM [dThumbnail]
        )
    AND [Brand Code] NOT IN ('DEL');
