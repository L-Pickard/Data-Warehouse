...
SELECT CAST(CAST(YEAR(MAX(gl.[Posting Date])) AS VARCHAR(4)) + RIGHT('0' + CAST(MONTH(
                    MAX(gl.[Posting Date])) AS VARCHAR(2)), 2) + RIGHT('0' + CAST(DAY(
                    MAX(gl.[Posting Date])) AS VARCHAR(2)), 2) AS INTEGER) AS 
    'Date Key'
    ...
    ,CAST(SUM(gl.[Amount]) AS DECIMAL(20, 8)) AS 'USD Amount'
    ,CAST(SUM(gl.[VAT Amount]) AS DECIMAL(20, 8)) AS 'USD VAT Amount'
    ,CAST(SUM(gl.[Debit Amount]) AS DECIMAL(20, 8)) AS 'USD Debit Amount'
    ,CAST(SUM(gl.[Credit Amount]) AS DECIMAL(20, 8)) AS 'USD Credit Amount'
    ,CAST(@End AS DATE) AS 'USD XR Date'
    ,CAST(1.00 AS DECIMAL(12, 8)) AS 'USD XR'
FROM [US Entry GL] AS gl
